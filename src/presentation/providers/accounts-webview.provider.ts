  ...
  public async refresh() {
    if (this._view) {
      try {
        ...
        Logger.getInstance().error('Webview refresh failed', err);
      }
    }
  }
    ...
    const getHeroStats = (acc: any) => {
      ...
      if (acc?.balances) {
        Object.keys(acc.balances).forEach(k => {
          ...
          if (l.includes('claude') && !l.includes('all')) stats.claude = acc.balances[k]?.value || 0;
          if (l.includes('pro') || k === 'google_one_ai') stats.pro = acc.balances[k]?.value || 0;
          if (l.includes('flash')) stats.flash = acc.balances[k]?.value || 0;
        });
      }
      return stats;
    };
